...
import {IoCartOutline, IoChevronDown, IoClose, IoDisc, IoHeartOutline} from "react-icons/io5";
    ...
    return <Page style={{paddingTop: 110, paddingBottom: 80, backgroundColor: '#F2F2F2'}} onScroll={(event) => {
                ...
                {menu.productId.map((productId,index,source) => {
                    ...
                    return <div key={productId} style={{
                        ...
                        <div style={{position: 'relative', marginLeft: 20}}>
                            ...
                            <StoreValue store={shoppingCart} selector={s => (s.filter(t => t.product.id === productId)?.reduce((total,item) => total + item.total,0) ?? 0)} property={'value'}>
                                <AddProductToCart onChange={(value) => {
                                    shoppingCart.setState(produce(s => {
                                        ...
                                        if (currentIndex >= 0) {
                                            ...
                                            s.push({product, total: value, totalPrice : value * product.price,options:[]});
                                        }
                                    }));
        ...
        <div style={{
            ...
            <div style={{display:'flex',backgroundColor:red,color:white,padding:'5px 20px',margin:'5px 10px',borderRadius:10}}>
                ...
                <div style={{display:'flex',alignItems:'flex-end'}}>
                    <div>AED 29.50</div>
                    <div style={{fontSize:10,marginLeft:5,marginBottom:1}}>inclusive of taxes</div>
                </div>
                </div>
            </div>
        </div>
                ...
                <Button onTap={async () => {
                    ...
                    if(errors.length > 0){
                        await showModal(closePanel => {
                            return <div style={{display:'flex',flexDirection:'column',padding:10,background:red,color:'white',borderRadius:10}}>
                                {errors.map(error => {
                                ...
                                <div style={{display:'flex',flexDirection:'row-reverse',marginTop:5}}>
                                    <Button onTap={() => {
                                        closePanel(true);
                                    }} title={'Ok'} icon={IoClose} theme={ButtonTheme.danger} style={{color:'white',fontSize:13,padding:'5px 10px'}}
