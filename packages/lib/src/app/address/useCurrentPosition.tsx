...
import {ButtonTheme, pageColor} from "../../components/Theme";
                ...
                if (state === 'prompt') {
                    const isOk = await appContext.showModal(closePanel => {
                        return <div style={{
                            ...
                            <div style={{fontSize: 20, marginBottom: 10}}>Request authorization to use your location.
                            </div>
                            <div style={{lineHeight: 1.5, marginBottom: 20}}>This application would like to request
                                permission to use
                                your current location in order to provide you with a more pleasant experience when it
                                comes to filling
                                out the information for your delivery address.
                            </div>
                            <div style={{display: 'flex', justifyContent: 'flex-end'}}>
                                <Button title={'OK'} icon={MdOutlineLocationOn} style={{border: 'none'}}
                                        theme={ButtonTheme.danger} onTap={() => closePanel(true)}/>
                                <Button title={'NO'} icon={MdOutlineCancel} style={{border: 'none'}}
                                        theme={ButtonTheme.subtle} onTap={() => closePanel(false)}/>
                            </div>
                        </div>
                    });
                    if (!isOk) {
