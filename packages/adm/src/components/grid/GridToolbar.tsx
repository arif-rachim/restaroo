...
export function GridToolbar<T>(props: { collection: string, collectionStore: Store<ListResult<BaseModel>> }) {
    ...
    return <div style={{display: 'flex', borderBottom: border}}>
        ...
        <ButtonSimple title={'Configure'} icon={IoSettings} onClick={async () => {
            const result: BaseModel | false = await showSlidePanel(closePanel => {
                return <GridConfig closePanel={closePanel} collectionOrConfigId={collection}/>
            }, {position: "right"});
            if (result === false) {
                return;
            }
