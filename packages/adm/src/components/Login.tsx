                ...
                <OnEnter onEnter={async (event: KeyboardEvent) => {
                    ...
                    <Card style={{padding: 20, marginBottom: 10, maxWidth: 400}}>
                        <Visible if={status === 'START'}>
                            <div style={{lineHeight: 1.5, marginTop: 10, marginBottom: 10}}>A one-time password (OTP)
                                will be sent to the mobile phone number you provided. Please double check that the
                                country code that corresponds to your phone number has been entered correctly.
                            </div>

                            <StoreValue store={store} selector={s => s.phoneNumber} property={'value'}>
                                <DInput titlePosition={'left'} style={{
                                    titleStyle: {margin: 0, padding: 0},
                                    containerStyle: {alignItems: 'flex-start'}
                                }} title={<StoreValue store={store} selector={s => s.countryCode} property={'value'}>
                                    <DInput titlePosition={'left'} titleWidth={100} title={'Phone Number'}
                                            placeholder={'Select Country Code'} onFocus={async () => {
                                        ...
                                        titleStyle: {margin: 0, padding: 0, marginRight: 5},
                                        inputStyle: {width: 60, marginRight: 5}
                                    }} readOnly={true}/>
                        ...
                        <Visible if={status === 'WAITING-OTP'}>
                            <div style={{
                                ...
                                marginTop: 10,
                                marginBottom: 10
                            }}>{`Please enter the 6-digit OTP to validate your request. An OTP message is being sent to your mobile phone ${fullPhoneNumber()}.`}</div>
                            <div style={{fontSize: 12, fontWeight: 'bold', marginBottom: 10}}>Please enter the otp :
                            </div>
                        ...
                        <Visible if={status === 'OTP-VALIDATION-FAILED'}>
                            <div style={{
                                ...
                                marginTop: 0,
                                marginBottom: 20
                            }}>{`OTP validation failed. Would you like to resend the OTP or send it to a different phone number? The previous OTP was sent to ${fullPhoneNumber()}.`}</div>
                            <div style={{display: 'flex'}}>
                                ...
                                }} title={'Resend OTP'} style={{width: '50%', fontSize: 12, marginRight: 10}}
                                         iconStyle={{fontSize: 16}} icon={IoRefresh} theme={ButtonTheme.danger}/>
                                <DButton onTap={() => {
                                    store.set(produce(s => {
                                        s.status = 'START';
                                        s.otp = '';
                                ...
                                }} title={'Resend to different Number'} style={{width: '50%', fontSize: 12, padding: 0}}
                                         iconStyle={{fontSize: 16}} icon={IoPhonePortrait} theme={ButtonTheme.danger}/>
                            </div>
                        </Visible>
                    </Card>
                </OnEnter>
                ...
                <OnEnter onEnter={() => {
                    ...
                    <Card style={{padding: 20, marginBottom: 10, width: 400}}>
                        <div style={{
                            ...
                            marginTop: 0,
                            marginBottom: 20
                        }}>{`You appear to be visiting this application for the first time; please provide your name to complete registration.`}</div>
                        <DInput title={'Name'} titlePosition={'left'} placeholder={'Enter your name'}
                                onChange={event => {
